...
import { FileWarning, Send, MapPin, MessageSquare, Camera, X, CheckCircle } from 'lucide-react';
import Card from '../components/Card';
import { useI18n, TranslationKey } from '../i18n';
import { submitUserReport } from '../services/adminDataService';
...
const ReportPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!issue || !priority || !location || !description) {
        ...
        try {
            ...
            const reportId = submitUserReport({
                type: 'incident',
                title: `${issue} - ${role} Report`,
                description,
                location,
                ...
                userInfo: {
                    id: `user-${Date.now()}`,
                    role: role.toLowerCase() as 'visitor' | 'security' | 'volunteer' | 'medical' | 'vendor',
                },
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-2xl max-w-md mx-4">
                        <div className="text-center">
                            ...
                            <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">Report Submitted!</h3>
                            <p className="text-gray-600 dark:text-gray-300">Your incident report has been sent to the admin panel for review.</p>
                        </div>
                    </div>
                </div>
            )}
                            ...
                            <label htmlFor="location" className="font-bold text-gray-800 dark:text-white">{t('reportDetails')}</label>
                             <div className="relative mt-2">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-10 p-3 rounded-lg bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-orange-500 text-gray-800 dark:text-gray-200" 
                                    required
                                />
                            </div>
                             <div className="relative mt-2">
